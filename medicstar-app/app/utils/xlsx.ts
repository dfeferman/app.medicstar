...
export function findFirstMatchingHeader(caseInsensitiveRow: Record<string, unknown>, candidates: string[]): unknown {
  for (const c of candidates) {
    if (c in caseInsensitiveRow) return caseInsensitiveRow[c];
  }
  return undefined;
}

export function convertToStringOrEmpty(value: unknown): string {
  if (value === null || value === undefined) return "";
  return String(value).trim();
}

export function toNullableString(value: unknown): string | null {
  const s = convertToStringOrEmpty(value);
  return s.length === 0 ? null : s;
}

export function convertToIntegerOrZero(value: unknown): number {
